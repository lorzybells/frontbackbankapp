...
export class UserService {
  ...
  transaction( From: Uint16Array, To: Uint16Array, Amount: Uint16Array): any {
    this.http.post(`${this.url}transaction`, {From, To, Amount },httpOptions).toPromise().then((res: any) => {
      if (res && res.jwt) {
        sessionStorage.setItem('jwt', res.jwt);
        this.errorSubject.next(null);
        if (res.data) {
